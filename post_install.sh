...
CFG_FLDR="/usr/local/etc/syncthing"

# Enable the service
sysrc -f /etc/rc.conf syncthing_enable="YES"

syncthing generate --gui-user=$USER --gui-password=$PASS --no-default-folder --config=$CFG_FLDR
sed -i '' 's|<address>127.0.0.1:8384</address>|<address>0.0.0.0:8384</address>|g' $CFG_FLDR/config.xml
chown syncthing:syncthing $CFG_FLDR/config.xml
